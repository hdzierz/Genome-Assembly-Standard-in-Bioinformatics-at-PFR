- GitHub repo containing the workflow. The workflow needs to run.
- FASTA of super-scaffolds / chromosomes
- FASTA of scaffolds
- FASTA of contigs
- FASTA repeat masked
- AGP files mapping contigs to scaffolds and scaffolds to super scaffolds
- GFF Repeats
- GFF with gene models where available
- GFF with liftovers where applicable
- Indices
- Linkage maps if used during scaffold assignment aif not otherwise included in a Git repo oth(Linkage maps (if used during scaffold assignment) in standard format or links to maps in Git